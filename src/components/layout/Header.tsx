...
import { Layers, Shield, Sparkles, Search, CreditCard, Activity, User, Lock } from 'lucide-react';

export const Header: React.FC<{ onOpenSearch: () => void }> = ({ onOpenSearch }) => {
  const location = useLocation();
  const { user, isAuthenticated, t } = useApp();
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          {isAuthenticated ? (
            ...
            <Link
              to="/login"
              className="px-4 py-2 rounded-xl bg-gold text-dark-bg text-xs font-bold shadow-gold-sm hover:brightness-110"
            >
              Sign In
            </Link>
          )}
